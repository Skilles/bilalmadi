			...
			<div className={container}>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 [&:has([data-experience-card]:not(:first-child):hover)_[data-experience-marker='primary']]:bg-line">
					{items.map((item, index) => (
						<article
							className={`${reveal} group relative pb-7 transition-transform duration-400 hover:-translate-y-0.5 md:px-7 md:pb-8 md:pt-2 lg:border-l lg:border-white/7 lg:first:border-l-0 lg:first:pl-0 ${
								index % 2 === 1 ? 'md:border-l md:border-white/7' : 'md:pl-0 lg:pl-7'
							} ${index > 1 ? 'border-t border-white/7 pt-8 md:mt-2 lg:mt-0 lg:border-t-0' : ''} ${
								index < items.length - 1 ? 'mb-7 border-b border-white/7 md:mb-0 md:border-b-0' : ''
							}`}
